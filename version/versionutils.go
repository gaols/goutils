...
func initVersion(source *VersionSource) error {
	...
	if goutils.IsBlank(strContent) {
		if goutils.IsNotBlank(source.InitialVersion) {
...
func (ver *VersionSource) check() error {
	if goutils.IsBlank(ver.SourcePath) {
		currentPath, err := os.Getwd()
		if err != nil {
			return err
		}
		ver.SourcePath = path.Join(currentPath, "version")
	}
	if !goutils.IsFileExists(ver.SourcePath) {
		return errors.New(fmt.Sprintf("Version file not found: %s ", ver.SourcePath))
	}
	if goutils.IsNotBlank(ver.InitialVersion) && !IsValidDigitalVersion(ver.InitialVersion) {
		return errors.New("Invalid initial version: " + ver.InitialVersion)
	}
...
func IsValidDigitalVersion(version string) bool {
	return goutils.IsNotBlank(version) && versionPatt.MatchString(version)
}
